...
//        }
    ...
    public String doLogin(HttpSession session, String uEmail, String uPwd){
        ...
        // 로그인 전 사용자가 가려던 페이지 확인
        String redirectURL = (String) session.getAttribute("redirectURL");
        if (redirectURL != null) {
            session.removeAttribute("redirectURL"); // 사용 후 삭제
            return Ut.jsReplace("S-A",Ut.f("%s님 환영합니다", user.getUNickName()), redirectURL);
        }


        return Ut.jsReplace("S-A",Ut.f("%s님 환영합니다", user.getUNickName()),"/usr/myPage/mypage");
    }
